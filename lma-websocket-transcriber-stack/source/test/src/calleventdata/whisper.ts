...
import { SocketCallData, AddTranscriptSegmentEvent, CallMetaData } from './eventtypes';
...
const USE_OPENAI_API = process.env['USE_OPENAI_API'] === 'true' || true;
...
export const startWhisperTranscribe = async (
    ...
    if (USE_OPENAI_API) {
        if (!OPENAI_API_KEY) {
            server.log.error(`[WHISPER]: [${callMetaData.callId}] - OpenAI API key not provided`);
        } else {
            server.log.info(`[WHISPER]: [${callMetaData.callId}] - Using OpenAI API with model ${OPENAI_MODEL}`);
        }
    }
    ...
    async function processAudioChunk(
        ...
        try {
            ...
            // Run Whisper on the WAV file - choose between OpenAI API and local model
            let transcriptionResult: WhisperTranscriptionResult;
            
            if (USE_OPENAI_API && openai) {
                try {
                    transcriptionResult = await runWhisperOpenAI(tempWavFile, server, callId);
                    server.log.debug(`[WHISPER]: [${callId}] - OpenAI API transcription successful`);
                } catch (openaiError) {
                    server.log.error(`[WHISPER]: [${callId}] - OpenAI API failed, falling back to local model: ${normalizeErrorForLogging(openaiError)}`);
                    transcriptionResult = await runWhisperOnFile(tempWavFile, tempJsonFile, server, callId);
                }
            } else {
                transcriptionResult = await runWhisperOnFile(tempWavFile, tempJsonFile, server, callId);
            }
...
// Run Whisper using OpenAI API
async function runWhisperOpenAI(
    inputFile: string,
    server: FastifyInstance,
    callId: string
    ...
    try {
        server.log.debug(`[WHISPER]: [${callId}] - Sending audio to OpenAI API`);
        
        // Create a readable stream from the file
        const audioFile = fs.createReadStream(inputFile);
        ...
        server.log.debug(`[WHISPER]: [${callId}] - OpenAI API response: ${JSON.stringify(response)}`);
        ...
        if (response.segments && Array.isArray(response.segments)) {
            result.segments = response.segments.map((segment: any, index: number) => ({
                id: index,
                seek: 0,
                start: segment.start,
                end: segment.end,
        ...
        server.log.error(`[WHISPER]: [${callId}] - Error in runWhisperOpenAI: ${normalizeErrorForLogging(error)}`);
        throw error;
    }
}
